...
namespace FFA.Services
{
    public sealed class ListService : Service
    {
        ...
        public string List<T, TSummary>(IEnumerable<T> elements, Func<T, string> nameSelector,
            Func<T, int> lengthSelector, Func<T, TSummary> summarySelector)
        {
            ...
            foreach (var element in elements.OrderBy(nameSelector))
                listBuilder.AppendFormat("{0}{1}{2}\n", Config.PREFIX, nameSelector(element).PadRight(padding),
                    summarySelector(element));
